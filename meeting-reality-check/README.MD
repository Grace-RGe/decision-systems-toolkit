...
- **Meeting design score** — 0–100, starting at 100 with three deductions: attendance, length, and productivity. Recurrence isn't scored on its own; it scales the attendance penalty, so a big meeting counts for more when it repeats than a right-sized one or a 1:1. The baselines for "right-sized" default from the purpose but are yours to adjust. The scoring rules are shown in the tool, in full.
...
- The score is a heuristic, not a verified instrument — 100 minus three deductions, kept deliberately simple so it can be explained and challenged. **Cost is not part of it:** an expensive meeting can be well designed, and a cheap one can be a waste.
